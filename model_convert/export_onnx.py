...
def parse_args():
    ...
    parser.add_argument("--encoder", "-e", type=str, required=False, default="./encoder.onnx", help="encoder onnx path")
    parser.add_argument("--decoder", "-d", type=str, required=False, default="./decoder.onnx", help="decoder onnx path")
...
def main():
    ...
    with torch.no_grad():
        ...
        torch.onnx.export(model,               # model being run
                        ...
                        encoder_path,              # where to save the model (can be a file or file-like object)
        ...
        sim_model,_ = simplify(encoder_path)
        onnx.save(sim_model, encoder_path)
        print(f"Export encoder to {encoder_path}")
        ...
        torch.onnx.export(model,               # model being run
                        inputs,                    # model input (or a tuple for multiple inputs)
                        decoder_path,              # where to save the model (can be a file or file-like object)
        ...
        sim_model,_ = simplify(decoder_path)
        onnx.save(sim_model, decoder_path)
        print(f"Export decoder to {decoder_path}")
